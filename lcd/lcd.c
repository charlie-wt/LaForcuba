...
void init_lcd() {
    ...
    XMCRB = _BV(XMM2) | _BV(XMM1);          /* XMCRA/B: External Memory Control Register A/B. Contain control bits for the External Memory Interface. */
                                            /* XMM1/2: External Memory High Mask. */
                                            /* Pretty sure this bit just enables external memory to be used, which in this case is that of the LCD display. */
    XMCRA = _BV(SRE);                       /* "External SRAM operation is enabled by setting the SRE bit in the XMCRA register." */
    DDRC |= _BV(RESET);                     /* Port C Data Direction Register. RESET = 7 */
    DDRB |= _BV(BLC);
    _delay_ms(1);
    PORTC &= ~_BV(RESET);
    _delay_ms(20);
...
void lcd_brightness(uint8_t i) {
    ...
    TCCR2A = _BV(COM2A1) | _BV(WGM21) | _BV(WGM20);     /* Timer/Counter Control Register 2: */
                                                            /* COM2A1 = 1: Clear OC2A on compare match, set at TOP. */
                                                            /* WGM21 & WGM20: Fast PWM. */
    TCCR2B = _BV(CS20);                                 /* Clock select for Timer/Counter to use; clk, no prescaling. */
    OCR2A = i;                                          /* Output Compare Register. */
}
